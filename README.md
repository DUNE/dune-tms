...
For an exact pre-#289 X-containment A/B comparison, set `Recon.Hough.UseLegacyXBarContainment = true`. This deliberately restores the old X-only long-dimension containment in Hough and extrapolation, including the old fixed Hough half-window. It is a diagnostic compatibility mode and should remain `false` for normal reconstruction.

Set `Recon.Hough.WriteDiagnosticHitSnapshots = true` as well to write `Hough_Diagnostic_Hits`, with direct plane/bar and Z/not-Z hit snapshots for the seed, post-walk, and post-DBSCAN states of rejected attempts. This is intended for event-display overlays and can grow output files substantially, so it is off by default.
