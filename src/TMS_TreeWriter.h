...
class TMS_TreeWriter {
  ...
  public:
    ...
    float RecoTrackHitBarType[__TMS_MAX_TRACKS__][__TMS_MAX_LINE_HITS__]; // Due to a lack of variables, but as this is taken from line hits, it would make sense (maybe times 2?)
  ...
  private:
    ...
    //YYYY Also need to add for Y info?
    ...
    float RecoHitBarType[__TMS_MAX_HITS__]; // Bartype 
