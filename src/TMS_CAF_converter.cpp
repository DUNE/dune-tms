...
caf::SRTMS TMS_CAF_converter::ConvertEvent(TMS_event &event) {
  ...
  for (auto it = LineCandidates.begin(); it != LineCandidates.end(); ++it) {
    ...
    caf::SRTrack track;
    track.start = firsthit_conv;
    track.end = lasthit_conv;
  ...
  for (auto it = HoughLines.begin(); it != HoughLines.end(); ++it, ++nit) {
    ...
    double xlow = Lines.second->Eval(zlow);
    double xhi = Lines.second->Eval(zhi);
    ...
    SRVector3D dir(xlen, 0, zlen); // Make the converted direction unit vector
    tracks[nit].dir = dir;
  }
