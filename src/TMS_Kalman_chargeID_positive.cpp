...
TMS_Kalman_plus::TMS_Kalman_plus() : 
...
TMS_Kalman_plus::TMS_Kalman_plus(std::vector<TMS_Hit> &Candidates, double charge) : 
  Bethe(Material::kPolyStyrene), 
  MSC(Material::kPolyStyrene),
  ForwardFitting(false),
  ...
  Talk(false)
{
  ...
  for (int i = 0; i < nCand; ++i) {
    ...
    {
      std::cerr << "[TMS_Kalman_plus.cpp] Hit " << i << "/" << nCand << " position not within TMS before Kalman filter, (x,y,z) = (" << x << ", " << y << ", " << z << ")" << std::endl;
      std::cerr << "[TMS_Kalman_plus.cpp] Were reco x and y values set before running Kalman?" << std::endl;
      //throw; // yeet it
    }
    ...
    if (abs(DeltaZ) > 1E-3) // TODO: Only add one hit per z for now, noise breaks
    {
      ...
      TMS_KalmanNode_plus Node(x, y, z, DeltaZ);
...
double TMS_Kalman_plus::GetKEEstimateFromLength(double startx, double endx, double startz, double endz) {
...
void TMS_Kalman_plus::RunKalman() {
...
void TMS_Kalman_plus::Update(TMS_KalmanNode_plus &PreviousNode, TMS_KalmanNode_plus &CurrentNode) {
...
void TMS_Kalman_plus::Predict(TMS_KalmanNode_plus &Node) {

  TMS_KalmanState_plus &PreviousState = Node.PreviousState;
  TMS_KalmanState_plus &CurrentState = Node.CurrentState;
  ...
  {
    //PreviousState.dxdz = TMS_Kalman_plus::AverageXSlope;
    if ( abs(TMS_Kalman_plus::AverageXSlope) > 2.0 )
    {
      std::cerr << "[TMS_Kalman_plus.cpp] Excessive average X slope = " << TMS_Kalman_plus::AverageXSlope << " of track (first to last hit), setting to 0" << std::endl;
  ...
  {
    if ( abs(TMS_Kalman_plus::AverageYSlope) > 1.5 )
    {
      std::cerr << "[TMS_Kalman_plus.cpp] Excessive average Y slope = " << TMS_Kalman_plus::AverageYSlope << " of track (first to last hit), setting to 0" << std::endl;
      PreviousState.dydz = 0.0;
    } else {
      PreviousState.dydz = TMS_Kalman_plus::AverageYSlope;
    }
  }
  ...
  if (p_2_up > 0) p_up = sqrt(p_2_up);
    ...
    std::cerr << "[TMS_Kalman_plus.cpp] negative momentum squared, setting momentum to 1 MeV" << std::endl;
    //p_up = 1;
    p_up = sqrt(en*en);
  }
  ...
  // {
  //   std::cerr << "[TMS_Kalman_plus.cpp] x value outside TMS: " << CurrentState.y << "\tTMS: [" << TMS_Const::TMS_Start[0] << ", "<< TMS_Const::TMS_End[0] << "]" << std::endl;
  //   if (Talk)
  //   {
  //     Node.PrintTrueReco();
  //     PreviousState.Print();
  ...
  // {
  //   std::cerr << "[TMS_Kalman_plus.cpp] y value outside TMS: " << CurrentState.y << "\tTMS: [" << TMS_Const::TMS_Start[1] << ", "<< TMS_Const::TMS_End[1] << "]" << std::endl;
  //   if (Talk)
  //   {
  //     Node.PrintTrueReco();
  //     PreviousState.Print();
...
TVectorD TMS_Kalman_plus::GetNoiseVector(TMS_KalmanNode_plus Node) {
