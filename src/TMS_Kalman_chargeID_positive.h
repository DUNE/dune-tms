#ifndef _TMS_KALMAN_CHARGEID_POSITIVE_H_SEEN_
#define _TMS_KALMAN_CHARGEID_POSITIVE_H_SEEN_
#include <iostream>
...
class TMS_KalmanState_plus {
  public:
    TMS_KalmanState_plus() = delete;
    TMS_KalmanState_plus(double xvar, double yvar, double zvar, double dxdzvar, double dydzvar, double qpvar) 
...
class TMS_KalmanNode_plus {
  public:
  TMS_KalmanNode_plus() = delete;

  // x,y,z, delta_z = distance from previous hit to current in z
  TMS_KalmanNode_plus(double xvar, double yvar, double zvar, double dzvar) :
  ...
  TMS_KalmanState_plus CurrentState;
  TMS_KalmanState_plus PreviousState;
  ...
  // void SetRecoXY(TMS_KalmanState_plus& State)
  // {
  //   RecoX = State.x;
  //   RecoY = State.y;
  // }
  ...
  bool operator<(const TMS_KalmanNode_plus &other) const {
    return z < other.z;
  }
  bool operator>(const TMS_KalmanNode_plus &other) const {
    return z > other.z;
...
class TMS_Kalman_plus {
  public:
    TRandom3 RNG;
    TMS_Kalman_plus();
    TMS_Kalman_plus(std::vector<TMS_Hit> &Candidates, double charge);
    ...
    std::vector<TMS_KalmanNode_plus> GetKalmanNodes() {return KalmanNodes;}

    TVectorD GetNoiseVector(TMS_KalmanNode_plus Node);
  ...
  private:
    ...
    void Predict(TMS_KalmanNode_plus &Node);
    void Update(TMS_KalmanNode_plus &PreviousNode, TMS_KalmanNode_plus &CurrentNode);
    void RunKalman();


    // State vector
    // x, y, dx/dz, dy/dz, q/p
    std::vector<TMS_KalmanNode_plus> KalmanNodes;
