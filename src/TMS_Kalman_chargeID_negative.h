#ifndef _TMS_KALMAN_CHARGEID_NEGATIVE_H_SEEN_
#define _TMS_KALMAN_CHARGEID_NEGATIVE_H_SEEN_
...
class TMS_KalmanState_minus {
  public:
    TMS_KalmanState_minus() = delete;
    TMS_KalmanState_minus(double xvar, double yvar, double zvar, double dxdzvar, double dydzvar, double qpvar) 
...
class TMS_KalmanNode_minus {
  public:
  TMS_KalmanNode_minus() = delete;

  // x,y,z, delta_z = distance from previous hit to current in z
  TMS_KalmanNode_minus(double xvar, double yvar, double zvar, double dzvar) :
  ...
  TMS_KalmanState_minus CurrentState;
  TMS_KalmanState_minus PreviousState;
  ...
  // void SetRecoXY(TMS_KalmanState_minus& State)
  // {
  //   RecoX = State.x;
  //   RecoY = State.y;
  // }
  ...
  bool operator<(const TMS_KalmanNode_minus &other) const {
    return z < other.z;
  }
  bool operator>(const TMS_KalmanNode_minus &other) const {
    return z > other.z;
...
class TMS_Kalman_minus {
  public:
    TRandom3 RNG;
    TMS_Kalman_minus();
    TMS_Kalman_minus(std::vector<TMS_Hit> &Candidates, double charge);
    ...
    std::vector<TMS_KalmanNode_minus> GetKalmanNodes() {return KalmanNodes;}

    TVectorD GetNoiseVector(TMS_KalmanNode_minus Node);
  ...
  private:
    ...
    void Predict(TMS_KalmanNode_minus &Node);
    void Update(TMS_KalmanNode_minus &PreviousNode, TMS_KalmanNode_minus &CurrentNode);
    void RunKalman();


    // State vector
    // x, y, dx/dz, dy/dz, q/p
    std::vector<TMS_KalmanNode_minus> KalmanNodes;
