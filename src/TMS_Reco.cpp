  ...
  UseClustering(TMS_Manager::GetInstance().Get_Reco_Clustering())
{
  ...
  //YYYY
  HoughLineY = new TF1("LinearHough3", "[0]+[1]*x", TMS_Const::TMS_Thin_Start, TMS_Const::TMS_Thick_Start);
...
//YYYY
  ...
  if (kTrackMethod == TrackMethod::kHough) {
    ...
    if (TMS_Manager::GetInstance().Get_Reco_HOUGH_FirstCluster()) {
      ...
      // DBScanCadiates: Clustered Hits bt DBSCAN
...
//YYYY
      ...
      //Filter the Hits from hit group by DBSCANed hits
...
//YYYY
      ...
      //Now TrackCandidates is the hits except clustered hits. like consider the clustered hit as one hit to opearate the hough transform
...
//  std::cout << "Masked (y) hits: " << MaskedY.size() << std::endl;
#endif
...
//  std::cout<<"1 in findtrack"<<std::endl;
...
//YYYY
  ...
  //std::cout<<"before kalman in findtrack"<<std::endl;
...
//  std::cout<<"after kalman in findtrack"<<std::endl;
        ...
        if (back_match) { 
          ...
          if (front_match) {
            ...
            while (sane) {  // Track seems to be backwards, so run adding of hits backwards
                // Sanity check for hits being in the detector volume (x and z)
                bool hit_outside = false;
                //if (std::abs(XTracks[itX].GetNotZ()) > 4000.0 || XTracks[itX].GetNotZ() == 0. 
                //        || XTracks[itX].GetZ() < 11000 || XTracks[itX].GetZ() > 20000) {
                //    --itX;
                //    hit_outside = true;
                //}
                if (XTracks[itX].GetNotZ() > 500.0 || XTracks[itX].GetNotZ() == 0. || XTracks[itX].GetNotZ() < -3500.0
                        || XTracks[itX].GetZ() < 11000 || XTracks[itX].GetZ() > 20000) {
                    --itX;
                    hit_outside = true;
                }
                ...
                if ((XTracks[itX]).GetPlaneNumber() == (YTracks[itY]).GetPlaneNumber()) {
                    ...
                    if (stereo_view) {
                        ...
                        YTracks[itY].SetRecoY(XTracks[itY].GetNotZ());
#ifdef DEBUG
                        std::cout << "same" << std::endl;
                        std::cout << "Hit: " << UTracks[itU].GetRecoX() << " | " << UTracks[itU].GetRecoY() << " | " << UTracks[itU].GetZ() << " than: " << VTracks[itV].GetRecoX() << " | " << VTracks[itV].GetRecoY() << " | " << VTracks[itV].GetZ() << std::endl;
#endif
                        ...
                        //TODO 일단 Plane# 같으면 그냥 하나의 값으로 통일 근데꽤 포지션 다른것들이 많음 if shower
                        if (itX > 0 && XTracks[itX].GetZ() == XTracks[itX - 1].GetZ()) {
                            XTracks[itX-1].SetRecoX(YTracks[itY].GetNotZ());
                            XTracks[itX-1].SetRecoY(XTracks[itX].GetNotZ());
                        ...
                        if (itY > 0 && YTracks[itY].GetZ() == YTracks[itY - 1].GetZ()) {
                            YTracks[itY-1].SetRecoX(YTracks[itY].GetNotZ());
                    ...
                    if (stereo_view) {
                        if (itX > 0 && itY > 0) {
                            ...
                            if (XTracks[itX].GetZ() == XTracks[itX - 1].GetZ()) {
                                XTracks[itX-1].SetRecoX(YTracks[itY].GetNotZ());
                                XTracks[itX-1].SetRecoY(XTracks[itX-1].GetNotZ());
                                //                      CalculateRecoY(XTracks[itX - 1], XTracks[itX - 1], YTracks[itY]);
                                //                      CalculateRecoX(XTracks[itX - 1], YTracks[itY], XTracks[itX - 1]);
                                (aTrack.Hits).push_back(XTracks[itX]);  // This adds the original hit
                                if (itX > 0) --itX; // and this allows for the other hit then to be added with the next push_back statement
                            }
                            (aTrack.Hits).push_back(XTracks[itX]);
                    ...
                    if (stereo_view) {
                        if (itY > 0 && itX > 0) {
                            ...
                            if (XTracks[itX].GetZ() == XTracks[itX - 1].GetZ()) {
                                ...
                                XTracks[itX-1].SetRecoY(XTracks[itX-1].GetNotZ());
                                //                      CalculateRecoY(XTracks[itX - 1], XTracks[itX - 1], YTracks[itY - 1]);
                                //                      CalculateRecoX(XTracks[itX - 1], YTracks[itY - 1], XTracks[itX - 1]);
                                (aTrack.Hits).push_back(XTracks[itX]);  // This adds the original hit
                                if (itX > 0) --itX; // and this allows for the other hit then to be added with the next push_back statement
                            }
                            (aTrack.Hits).push_back(XTracks[itX]);
                            ...
                            XTracks[itX].SetRecoX(YTracks[itY].GetNotZ());
                            XTracks[itX].SetRecoY(XTracks[itX].GetNotZ());
                            //                    CalculateRecoY(XTracks[itX], XTracks[itX], YTracks[itY]);
                            //                    CalculateRecoX(XTracks[itX], YTracks[itY], XTracks[itX]);
                            if (XTracks[itX].GetZ() == XTracks[itX - 1].GetZ()) {
                                XTracks[itX-1].SetRecoX(YTracks[itY].GetNotZ());
                                XTracks[itX-1].SetRecoY(XTracks[itX-1].GetNotZ());
                                //                      CalculateRecoY(XTracks[itX - 1], XTracks[itX - 1], YTracks[itY]);
                            ...
                            YTracks[itY].SetRecoX(YTracks[itY].GetNotZ());
                            YTracks[itY].SetRecoY(XTracks[itX].GetNotZ());
                            //                    CalculateRecoY(YTracks[itY], XTracks[itX], YTracks[itY]);
                            //                    CalculateRecoX(XTracks[itX], YTracks[itY], YTracks[itY]);
                            if (YTracks[itY].GetZ() == YTracks[itY - 1].GetZ()) {
                                YTracks[itY-1].SetRecoX(YTracks[itY-1].GetNotZ());
                                YTracks[itY-1].SetRecoY(XTracks[itX].GetNotZ());
                                //                      CalculateRecoY(YTracks[itY - 1], XTracks[itX], YTracks[itY - 1]);
                                //                      CalculateRecoX(XTracks[itX], YTracks[itY - 1], YTracks[itY - 1]);
                                (aTrack.Hits).push_back(YTracks[itY]);  // This adds the original hit
                                if (itY > 0) --itY; // and this allows for the other hit then to be added with the next push_back statement
                            }
                            (aTrack.Hits).push_back(YTracks[itY]);
...
std::vector<TMS_Hit> TMS_TrackFinder::RunHough(const std::vector<TMS_Hit> &TMS_Hits, const char &hitgroup) {
    ...
    //YYYY
    //Question: isn't it YBar instead of XBar?
    //original  bool IsXZ = (TMS_Hits.front().GetBar().GetBarType() == TMS_Bar::kUBar || TMS_Hits.front().GetBar().GetBarType() == TMS_Bar::kVBar || TMS_Hits.front().GetBar().GetBarType() == TMS_Bar::kXBar);
    bool IsXZ = (TMS_Hits.front().GetBar().GetBarType() == TMS_Bar::kUBar || TMS_Hits.front().GetBar().GetBarType() == TMS_Bar::kVBar || TMS_Hits.front().GetBar().GetBarType() == TMS_Bar::kYBar);
    ...
    if (VariableHough) {
        ...
        //YYYY
        if (hitgroup == 'Y') { 
            min_notz=TMS_Const::TMS_End_Exact[1];
            max_notz=TMS_Const::TMS_End_Exact[1];
        }
        bool print = true;
...
//if match the Bartype, then push_back?
...
void TMS_TrackFinder::WalkDownStream(std::vector<TMS_Hit> &vec, std::vector<TMS_Hit> &full) {
    ...
    for (std::vector<TMS_Hit>::size_type i = 1; i < size; ++i) {
        ...
        double grad_exp = (y - yprev) / (x - xprev);//deltaX/deltaZ
                ...
                if (fabs(grad_new - grad_exp) <= 1.5) {
                    ...
                    if (xcand <= vec.back().GetPlaneNumber()) {
                        ...
                        for (std::vector<TMS_Hit>::iterator jt = vec.begin()+i; jt != vec.end(); ++jt) {
                            ...
                            if (xcand <= compx) { //xcand=full's plane number
                                vec.insert(jt+1, std::move(hits));
                                break;
                            } else if (xcand > compx && xcand <= compx1) {
                                // Put in the previous
