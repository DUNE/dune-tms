...
//TODO new orientation
  ...
  for (std::vector<TMS_Hit>::iterator it = HitPool.begin(); it != HitPool.end();) {
    ...
    // Accept Hough points within a configurable number of transverse half-widths.
    // The default 1.5 reproduces the previous three Contains checks exactly.
    const double containment_half_width = hit.GetBar().GetBarType() == TMS_Bar::kXBar ?
        TMS_Manager::GetInstance().Get_Reco_HOUGH_ContainmentHalfWidthX() :
        TMS_Manager::GetInstance().Get_Reco_HOUGH_ContainmentHalfWidthY();
    if (std::abs(HoughPoint - bar.GetNotZ()) <= containment_half_width * bar.GetNotZw() &&
        std::abs(zhit - bar.GetZ()) <= bar.GetZw() / 2.0) {
...
std::vector<TMS_Hit> TMS_TrackFinder::Extrapolation(const std::vector<TMS_Hit> &TrackHits, const std::vector<TMS_Hit> &Hitpool) {
  ...
  for (std::vector<TMS_Hit>::const_iterator it = Hitpool.begin(); it != Hitpool.end(); ++it) {
    ...
    if ((*it).GetZ() > TrackHits.back().GetZ()) {
      ...
      const double containment_width_scale = (*it).GetBar().GetBarType() == TMS_Bar::kXBar ?
          TMS_Manager::GetInstance().Get_Reco_EXTRAPOLATION_ContainmentWidthScaleX() :
          TMS_Manager::GetInstance().Get_Reco_EXTRAPOLATION_ContainmentWidthScaleY();
      bool CloseBars = ((*it).GetNotZ() <= ((*it).GetZ() * end.slope + end.intercept + TMS_Manager::GetInstance().Get_Reco_EXTRAPOLATION_NumBarsEnd() * containment_width_scale * (*it).GetBar().GetNotZw()) &&
            (*it).GetNotZ() >= ((*it).GetZ() * end.slope + end.intercept - TMS_Manager::GetInstance().Get_Reco_EXTRAPOLATION_NumBarsEnd() * containment_width_scale * (*it).GetBar().GetNotZw()));
      if (CloseBars) {
    ...
    for (std::vector<TMS_Hit>::const_iterator it = Hitpool.begin(); it != Hitpool.end(); ++it) {
      ...
      if ((*it).GetZ() < returned.front().GetZ()) {
        ...
        const double containment_width_scale = (*it).GetBar().GetBarType() == TMS_Bar::kXBar ?
            TMS_Manager::GetInstance().Get_Reco_EXTRAPOLATION_ContainmentWidthScaleX() :
            TMS_Manager::GetInstance().Get_Reco_EXTRAPOLATION_ContainmentWidthScaleY();
        bool CloseBars = ((*it).GetNotZ() <= ((*it).GetZ() * front.slope + front.intercept + TMS_Manager::GetInstance().Get_Reco_EXTRAPOLATION_NumBarsStart() * containment_width_scale * (*it).GetBar().GetNotZw()) &&
              (*it).GetNotZ() >= ((*it).GetZ() * front.slope + front.intercept - TMS_Manager::GetInstance().Get_Reco_EXTRAPOLATION_NumBarsStart() * containment_width_scale * (*it).GetBar().GetNotZw()));
        if (CloseBars) {
          // Calculate temporary node to check for distance
          aNode candidate((*it).GetPlaneNumber(), (*it).GetBarNumber());
