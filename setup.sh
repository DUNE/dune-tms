...
# Double check that we're not on Alma9
grep AlmaLinux /etc/redhat-release > /dev/null 2>&1 
if [ $? ]; then
  echo "You appear to be running on AlmaLinux9, you should use setup_FNAL.sh instead :)"
else
