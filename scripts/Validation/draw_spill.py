...
def inside_tms(x, y, z):
    ...
    if not -4000 < x < 4000:
        is_inside = False
    if not -1548 - 4000 < y < -1548 + 4000:
        is_inside = False
    if not 11000 < z < 18000:
        is_inside = False
    return is_inside
...
def inside_lar(x, y, z):
    is_inside = True
    if not -3478.48 < x < 3478.48:
        is_inside = False
    if not -2166 < y < -829.282:
        is_inside = False
    if not 4179.24 < z < 9135.88:
        is_inside = False
    return is_inside
...
def draw_spill(
    ...
    if not os.path.exists(input_filename):
        raise ValueError(f"Cannot find input_filename {input_filename}")
    if readout_filename != "" and not os.path.exists(readout_filename):
        raise ValueError(f"Cannot find readout_filename {readout_filename}")
    if spill_number < -1:
        raise ValueError(f"Got spill_number = {spill_number}")
    if time_slice < -1:
        raise ValueError(f"Got time_slice = {time_slice}")
    ...
    assert c.GetEntries() > 0, (
        "Didn't get any entries, are you sure the input_filename is right?\n"
        + input_filename
    )

    truth = ROOT.TChain("Truth_Info")
    truth.Add(input_filename)
    assert truth.GetEntries() > 0, (
        "Didn't get any entries in Truth_Info, are you sure the input_filename is right?\n"
        + input_filename
    )
    ...
    assert truth.GetEntries() > 0, (
        "Didn't get any entries in Truth_Spill, are you sure the input_filename is right?\n"
        + input_filename
    )
    ...
    if use_readout:
        ...
        assert readout.GetEntries() > 0, (
            "Didn't get any entries in TMS, are you sure the readout_filename is right?\n"
            + readout_filename
        )

    max_n_spills = (
        10000  # TODO add some meta info to output file with n spill info for file
    )

    haxis = ROOT.TH2D(
        "hits_in_event",
        ";Position Z (m);Position X (m);Total Energy (MeV)",
        100,
        bound_z_min,
        bound_z_max,
        100,
    ...
    for current_spill_number in range(max_n_spills):
        htime = ROOT.TH1D(
            f"htime_{current_spill_number}",
            ";Time (ns);Total Hit E (MeV)",
            500,
            -200,
            12000,
        )
        ...
        max_hit_energy = 50
        henergy = ROOT.TH1D(
            f"henergy_{current_spill_number}",
            ";Hit E (MeV);N Hits",
            50,
            0,
            max_hit_energy,
        ...
        for i in range(n_events):
            ...
            if spill_number < current_spill_number:
                continue
            if spill_number > current_spill_number:
                break
            if event == None:
            ...
            if only_true_tms_muons:
                assert (
                    truth != None
                ), "Truth shouldn't be None inside only_true_tms_muons"
                ...
                if not end_inside_tms:
                    continue
                print(
                    f"Muon Start XYZ: ({mx:0.2f}, {my:0.2f}, {mz:0.2f})\tMuon End XYZ: ({mdx:0.2f}, {mdy:0.2f}, {mdz:0.2f})\tstart_inside_tms: {start_inside_tms}\tend_inside_tms: {end_inside_tms},\tPDG: {truth.LeptonPDG}"
                )

            if only_true_lar_muons:
                assert (
                    truth != None
                ), "Truth shouldn't be None inside only_true_lar_muons"
                mx = truth.Muon_Vertex[0]
                my = truth.Muon_Vertex[1]
                mz = truth.Muon_Vertex[2]
                mdx = truth.Muon_Death[0]
            ...
            for hit in range(event.nHits):
                ...
                color_for_slice = (
                    ROOT.kGray
                    if reco_hit_slice == 0
                    else slice_colors[reco_hit_slice % len(slice_colors)]
                    + reco_hit_slice // len(slice_colors)
                    - 3
                )
                ...
                henergy.Fill(
                    reco_hit_energy
                    if reco_hit_energy < max_hit_energy
                    else 0.95 * max_hit_energy
                )
                x = reco_hit_z
                y = reco_hit_x
        ...
        for hit in range(truth_spill.TrueNonTMSNHits):
            ...
            if vid in vertex_id_mapping:
                ...
                color = (
                    ROOT.kGray
                    if vid not in vertex_id_mapping
                    else vertex_id_mapping[vid]
                )
        ...
        def draw(output_filename, markers, text_to_draw):
            ...
            if total_energy > minimum_energy_to_print:
                print(
                    f"Spill {current_spill_number},\tTime range: {time_low:0.0f}-{time_high:0.0f} ns,\tEnergy: {total_energy:0.2f} MeV,\tN hits: {n_hits}"
                )
                pad1.cd()
                ...
                M = 1e-3
                box_lar = make_box(
                    (lar_start * M, lar_x_start * M, lar_end * M, lar_x_end * M)
                )
                box_lar_had = make_box(
                    (
                        lar_start * M + 0.3,
                        lar_x_start * M + 0.3,
                        lar_end * M - 0.3,
                ...
                box_tms = make_box(
                    (tms_start * M, tms_x_start * M, tms_end * M, tms_x_end * M)
                )
                box_lar.Draw()
                box_lar_had.Draw()
        ...
        output_filename = os.path.join(
            out_dir, f"{name}_{current_spill_number:03d}.png"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-o",
        type=str,
        help="The output dir. Will be made if it doesn't exist. Default = spills/",
        default="spills",
    )
    parser.add_argument(
        "--name",
        "-n",
        type=str,
        help="The name of the output files. Will be <name>_<spill>_<slice>.png. Default = spill",
        default="spill",
    )
    parser.add_argument(
        "--input_filename", "-f", type=str, help="The file with the events to draw"
    )
    parser.add_argument(
        "--spillnum", "-s", type=int, help="The spill to draw. -1 for all", default=-1
    )
    parser.add_argument(
        "--timeslice",
        "-t",
        type=int,
        help="The time slice to draw. -1 for all",
        default=-1,
    )
    parser.add_argument(
        "--readout_filename",
        "-r",
        type=str,
        help="(optional) A file with the raw readout.",
        default="",
    )
    parser.add_argument(
        "--interactions",
        "-i",
        type=str,
        help="(optional) The subdet with interactions",
        default="",
    )
