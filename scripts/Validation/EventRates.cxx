// Add scope to avoid cross talk with other scripts
{
  ...
  if (true) { // on_new_spill || lar_shell_cut // shell energy plot needs every event
    ...
    for (int ip = 0; ip < truth.nTrueParticles; ip++) {
      ...
      // Check that this particle truly came from interaction added up in shell cut
      bool particle_from_shell_cut_interaction = truth.VertexIdOfMostEnergyInEvent == truth.VertexID[ip];
      ...
      if (is_nd_physics_muon && lar_shell_cut && particle_from_shell_cut_interaction) {
        // This is the full ND physics sample
        GetHist("event_rates__tms__tms_event_rates_with_shell_cut", "TMS Event Rates",
          "tms_event_rate")->Fill(0);
        if (tms_touch) GetHist("event_rates__tms__tms_event_rates_with_shell_cut", "TMS Event Rates",
          "tms_event_rate")->Fill(1);
        if (tms_end) GetHist("event_rates__tms__tms_event_rates_with_shell_cut", "TMS Event Rates",
          "tms_event_rate")->Fill(2);
        if (tms_touch && !tms_end) GetHist("event_rates__tms__tms_event_rates_with_shell_cut", "TMS Event Rates",
          "tms_event_rate")->Fill(3);
        GetHist("event_rates__tms__validation__shell_energy_cut", "LAr Visible Hadronic Energy from Primary Vertex in 30cm Shell",
      ...
      if (is_nd_physics_muon && particle_from_shell_cut_interaction) {
        GetHist("event_rates__tms__validation__shell_energy", "LAr Visible Hadronic Energy from Primary Vertex in 30cm Shell",
          "LArOuterShellEnergyFromVertex")->Fill(truth.LArOuterShellEnergyFromVertex);
      }
  ...
  for (int it = 0; it < truth.RecoTrackN; it++) {
    ...
    if (is_nd_physics_muon && lar_shell_cut) {
      GetHist("event_rates__tms__tms_muon_reco_rates_with_shell_cut", "TMS N Muons Reconstructed",
        "tms_muon_reco_rates")->Fill(0);
      if (tms_touch) GetHist("event_rates__tms__tms_muon_reco_rates_with_shell_cut", "TMS N Muons Reconstructed",
        "tms_muon_reco_rates")->Fill(1);
      if (tms_end) GetHist("event_rates__tms__tms_muon_reco_rates_with_shell_cut", "TMS N Muons Reconstructed",
        "tms_muon_reco_rates")->Fill(2);
      if (tms_touch && !tms_end) GetHist("event_rates__tms__tms_muon_reco_rates_with_shell_cut", "TMS N Muons Reconstructed",
        "tms_muon_reco_rates")->Fill(3);
    }
    if (lar_start && ismuon) {
    ...
    if (lar_fiducial_start && lar_shell_cut) {
      GetHist("event_rates__reco_pdg__pdg_with_shell_cut", "Primary PDG Reconstructed",
        "pdg")->Fill(PDGtoIndex(truth.RecoTrackPrimaryParticlePDG[it]));
      GetHist("event_rates__reco_pdg__fiducial_comparison_pdg_nostack_2_nd_physics", "Primary PDG Reco: ND Physics Muons",
        "pdg")->Fill(PDGtoIndex(truth.RecoTrackPrimaryParticlePDG[it]));
      GetHist("event_rates__tms__validation__shell_energy_reco_muons_cut", "LAr Visible Hadronic Energy from Primary Vertex in 30cm Shell",
        "LArOuterShellEnergyFromVertex")->Fill(truth.LArOuterShellEnergyFromVertex);
    }
    if (lar_fiducial_start) {
      GetHist("event_rates__tms__validation__shell_energy_reco_muons", "LAr Visible Hadronic Energy from Primary Vertex in 30cm Shell",
        "LArOuterShellEnergyFromVertex")->Fill(truth.LArOuterShellEnergyFromVertex);
    }
    if (lar_start) {
