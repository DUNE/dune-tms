...
Long64_t PrimaryLoop(Truth_Info& truth, Reco_Tree& reco, Line_Candidates& lc, int numEvents, TFile& outputFile) {
   ...
   fChain->SetBranchAddress("TrackHitEnergies", TrackHitEnergies, &b_RecoTrackHitEnergies); */
      ...
      /*if (reco.nTracks == 1) {
        DrawSlice(TString::Format("entry_%lld", entry_number).Data(), "lar_starting_single_track", 
                  TString::Format("n tracks = %d", reco.nTracks).Data(), reco, lc, truth, DrawSliceN::tons);
      ...
      if (reco.nTracks == 1) {
        ...
        if (ismuon) {
          DrawSlice(TString::Format("entry_%lld", entry_number).Data(), "single_muon", 
                    TString::Format("n tracks = %d", reco.nTracks).Data(), reco, lc, truth, DrawSliceN::tons);
          if (truth.RecoTrackPrimaryParticleTMSFiducialEnd[0]) 
            DrawSlice(TString::Format("entry_%lld", entry_number).Data(), "single_muon_tms_end", 
                    TString::Format("n tracks = %d", reco.nTracks).Data(), reco, lc, truth, DrawSliceN::tons);
      ...
      if (reco.nTracks > 2) {
        DrawSlice(TString::Format("entry_%lld", entry_number).Data(), "3_or_more_track", 
                  TString::Format("n tracks = %d", reco.nTracks).Data(), reco, lc, truth, DrawSliceN::many);
      }
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <input_filename> <num_events (optional)> <num slices to draw (optional)>" << std::endl;
        return 1;
    }
    ...
    if (argc > 3) DrawSliceN::max_slices = atoi(argv[3]);
