...
{
  REGISTER_AXIS(endpoint_dz, std::make_tuple("dZ (reco - true) (cm)", 31, -100.0, 50.0));
  REGISTER_AXIS(endpoint_dx, std::make_tuple("dX (reco - true) (cm)", 31, -50.0, 50.0));
  REGISTER_AXIS(endpoint_dy, std::make_tuple("dY (reco - true) (cm)", 31, -50.0, 50.0));
  REGISTER_AXIS(endpoint_dboth, std::make_tuple("2D XZ resolution (reco - true) (cm)", 31, 0, 100.0));
  for (int it = 0; it < reco.nTracks; it++) {
    ...
    if (end_tms && is_muon) has_contained_muon = true;
    
    if (has_contained_muon) {
        double true_endpoint_x = truth.RecoTrackPrimaryParticleTruePositionEnd[it][0];
        double reco_endpoint_x = reco.EndPos[it][0];
        double true_endpoint_y = truth.RecoTrackPrimaryParticleTruePositionEnd[it][1];
        double reco_endpoint_y = reco.EndPos[it][1];
        double true_endpoint_z = truth.RecoTrackPrimaryParticleTruePositionEnd[it][2];
        ...
        double endpoint_dxz = std::sqrt(endpoint_dz*endpoint_dz + endpoint_dx*endpoint_dx);
        GetHist("reco_track__track_resolution__endpoint_z",
                 "Endpoint Resolution dz", "endpoint_dz")->Fill(endpoint_dz);
        GetHist("reco_track__track_resolution__endpoint_x",
                 "Endpoint Resolution dx", "endpoint_dx")->Fill(endpoint_dx);
        GetHist("reco_track__track_resolution__endpoint_y",
                 "Endpoint Resolution dy", "endpoint_dy")->Fill(endpoint_dy);
        GetHist("reco_track__track_resolution__endpoint_xz",
                 "Endpoint Resolution 2d", "endpoint_dboth")->Fill(endpoint_dxz);
