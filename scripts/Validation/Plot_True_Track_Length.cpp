...
Long64_t PrimaryLoop(Truth_Info &truth, Reco_Tree &reco, Line_Candidates &lc,
  ...
  TH1D hist_muon_true_track_length(
      "muon_true_track_length",
      "True Areal Density;True Areal Density (g/cm2);N Muons",
      n_areal_density_bins, 0, areal_density_max);
  TH1D hist_muon_true_track_length_ignore_y(
      "muon_true_track_length_ignore_y",
      "True Areal Density, Ignoring Y;True Areal Density (g/cm2);N Muons",
      n_areal_density_bins, 0, areal_density_max);
  TH1D hist_muon_true_track_length_ratio("muon_true_track_length_ratio",
                                         "True Areal Density Ratio w/ and w/o "
                                         "Y;Ratio (ignore y/include y);N Muons",
                                         n_areal_density_bins, 0, 2);

  // Comparisons to reco tracks
  TH1D hist_reco_track_length(
      "reco_track_length",
      "Reco Areal Density;Reco Areal Density (g/cm2);N Muons",
      n_areal_density_bins, 0, areal_density_max);
  TH1D hist_true_track_length(
      "true_track_length",
      "True Areal Density;True Areal Density (g/cm2);N Muons",
      n_areal_density_bins, 0, areal_density_max);
  TH1D hist_true_track_length_original(
      "true_track_length_original",
      "True Areal Density;True Areal Density (g/cm2);N Muons",
      n_areal_density_bins, 0, areal_density_max);
  TH2D hist_reco_vs_true_track_length(
      "reco_vs_true_track_length",
      "Areal Density Smearing Matrix;True Areal Density (g/cm2);Reco Areal "
      "Density (g/cm2);N Muons",
      n_areal_density_bins, 0, areal_density_max, n_areal_density_bins, 0,
      areal_density_max);
  TH2D hist_reco_u_vs_true_track_length(
      "reco_u_vs_true_track_length",
      "Areal Density Smearing Matrix;True Areal Density (g/cm2);Reco U Areal "
      "Density (g/cm2);N Muons",
      n_areal_density_bins, 0, areal_density_max, n_areal_density_bins, 0,
      areal_density_max);
  TH2D hist_reco_v_vs_true_track_length(
      "reco_v_vs_true_track_length",
      "Areal Density Smearing Matrix;True Areal Density (g/cm2);Reco V Areal "
      "Density (g/cm2);N Muons",
      n_areal_density_bins, 0, areal_density_max, n_areal_density_bins, 0,
      areal_density_max);
  TH2D hist_reco_v_vs_reco_track_length(
      "reco_v_vs_reco_track_length",
      "Areal Density Reco Comparison;Reco Areal Density (g/cm2);Reco V Areal "
      "Density (g/cm2);N Muons",
      n_areal_density_bins, 0, areal_density_max, n_areal_density_bins, 0,
      areal_density_max);
  TH2D hist_reco_v_vs_reco_u_track_length(
      "reco_v_vs_reco_u_track_length",
      "Areal Density Reco Comparison;Reco U Areal Density (g/cm2);Reco V Areal "
      "Density (g/cm2);N Muons",
      n_areal_density_bins, 0, areal_density_max, n_areal_density_bins, 0,
      areal_density_max);
  TH2D hist_reco_vs_true_track_length_original(
      "reco_vs_true_track_length_original",
      "Areal Density Smearing Matrix;True Areal Density (g/cm2);Reco Areal "
      "Density (g/cm2);N Muons",
      n_areal_density_bins, 0, areal_density_max, n_areal_density_bins, 0,
      areal_density_max);
  TH2D hist_true_track_length_vs_density(
      "true_track_length_vs_density",
      "True Track Length Vs Areal Density;True Track Length (mm);True Areal "
      "Density (g/cm2);N Muons",
      n_length_bins, 0, length_max, n_areal_density_bins, 0, areal_density_max);
  TH2D hist_true_track_length_vs_density_original(
      "true_track_length_vs_density_original",
      "True Track Length Vs Areal Density;True Track Length (mm);True Areal "
      "Density (g/cm2);N Muons",
      n_length_bins, 0, length_max, n_areal_density_bins, 0, areal_density_max);
  TH2D hist_reco_track_length_vs_density(
      "reco_track_length_vs_density",
      "Reco Track Length Vs Areal Density;Reco Track Length (mm);Reco Areal "
      "Density (g/cm2);N Muons",
      n_length_bins, 0, length_max, n_areal_density_bins, 0, areal_density_max);

  TH2D hist_reco_vs_true_track_actual_length(
      "reco_vs_true_track_actual_length",
      "Track Length Smearing Matrix;True Track Length (mm);Reco Track Length "
      "(mm);N Muons",
  ...
  double y_min = -4000;
  double y_max = 1000;
  TH2D pseudo_event_yz_display_reco(
      "pseudo_event_yz_display_reco",
      "All Reco Track Hits;Reco Hit Z (mm);Reco Hit Y (mm);N Hits", n_points,
      11000, 19000, n_points, y_min, y_max);
  TH2D pseudo_event_yz_display_true(
      "pseudo_event_yz_display_true",
      "All True Track Hits;True Hit Z (mm);True Hit Y (mm);N Hits", n_points,
      11000, 19000, n_points, y_min, y_max);

  double x_max = 4000;
  TH2D pseudo_event_xz_display_reco(
      "pseudo_event_xz_display_reco",
      "All Reco Track Hits;Reco Hit Z (mm);Reco Hit Y (mm);N Hits", n_points,
      11000, 19000, n_points, -x_max, x_max);
  TH2D pseudo_event_xz_display_true(
      "pseudo_event_xz_display_true",
      "All True Track Hits;True Hit Z (mm);True Hit Y (mm);N Hits", n_points,
      11000, 19000, n_points, -x_max, x_max);

  int n_diff_bins = 101;
  TH2D hist_hit_reco_vs_true_x(
      "hit_reco_vs_true_x",
      "Track Hit Position Comparison X;True Hit Position X (mm);Reco Hit "
      "Position X (mm);N Hits",
      n_points, -x_max, x_max, n_points, -x_max, x_max);
  TH2D hist_hit_reco_vs_true_y(
      "hit_reco_vs_true_y",
      "Track Hit Position Comparison Y;True Hit Position Y (mm);Reco Hit "
      "Position Y (mm);N Hits",
      n_points, y_min, y_max, n_points, y_min, y_max);
  TH2D hist_hit_reco_vs_true_z(
      "hit_reco_vs_true_z",
      "Track Hit Position Comparison Z;True Hit Position Z (mm);Reco Hit "
      "Position Z (mm);N Hits",
      n_points, 11000, 19000, n_points, 11000, 19000);
  TH1D hist_hit_x_diff(
      "hit_x_diff",
      "Track Hit Position Comparison X;Reco - True Hit Position X (mm);N Hits",
      n_diff_bins, -1000, 1000);
  TH1D hist_hit_y_diff(
      "hit_y_diff",
      "Track Hit Position Comparison Y;Reco - True Hit Position Y (mm);N Hits",
      n_diff_bins, -1000, 1000);
  TH1D hist_hit_z_diff(
      "hit_z_diff",
  ...
  for (; entry_number < truth.GetEntriesFast() &&
    ...
    for (int itrack = 0; itrack < truth.RecoTrackN; itrack++) {
      ...
      if (is_muon && is_TMS_contained && is_TMS_contained_start) {
        ...
        hist_muon_true_track_length.Fill(
            truth.RecoTrackPrimaryParticleTrueTrackLength[itrack]);
        hist_muon_true_track_length_ignore_y.Fill(
            truth.RecoTrackPrimaryParticleTrueTrackLengthIgnoreY[itrack]);
        double ratio =
            truth.RecoTrackPrimaryParticleTrueTrackLengthIgnoreY[itrack] /
            truth.RecoTrackPrimaryParticleTrueTrackLength[itrack];
        if (ratio > 1)
        ...
        hist_reco_vs_true_track_length.Fill(
            truth.RecoTrackPrimaryParticleTrueTrackLength[itrack],
            reco.Length[itrack]);
        hist_reco_u_vs_true_track_length.Fill(
            truth.RecoTrackPrimaryParticleTrueTrackLength[itrack],
            lc.TrackLengthU[itrack]);
        hist_reco_v_vs_true_track_length.Fill(
            truth.RecoTrackPrimaryParticleTrueTrackLength[itrack],
            lc.TrackLengthV[itrack]);
        hist_reco_v_vs_reco_track_length.Fill(reco.Length[itrack],
                                              lc.TrackLengthV[itrack]);
        hist_reco_v_vs_reco_u_track_length.Fill(lc.TrackLengthU[itrack],
                                                lc.TrackLengthV[itrack]);

        hist_reco_vs_true_track_length_original.Fill(truth.Muon_TrueTrackLength,
                                                     reco.Length[itrack]);
        hist_true_track_length.Fill(
            truth.RecoTrackPrimaryParticleTrueTrackLength[itrack]);
        hist_true_track_length_original.Fill(truth.Muon_TrueTrackLength);

        TVector3 start(truth.Muon_Vertex[0], truth.Muon_Vertex[1],
                       truth.Muon_Vertex[2]);
        TVector3 end(truth.Muon_Death[0], truth.Muon_Death[1],
                     truth.Muon_Death[2]);
        double true_length = (end - start).Mag();
        hist_true_track_length_vs_density.Fill(
            true_length, truth.RecoTrackPrimaryParticleTrueTrackLength[itrack]);
        hist_true_track_length_vs_density_original.Fill(
            true_length, truth.Muon_TrueTrackLength);

        TVector3 reco_start(reco.StartPos[itrack][0], reco.StartPos[itrack][1],
                            reco.StartPos[itrack][2]);
        TVector3 reco_end(reco.EndPos[itrack][0], reco.EndPos[itrack][1],
                          reco.EndPos[itrack][2]);
        double reco_length = (reco_end - reco_start).Mag();
        hist_reco_track_length_vs_density.Fill(reco_length,
                                               reco.Length[itrack]);
        ...
        if (entry_number % sample_rate == 0) {
          ...
          for (int h = 0; h < truth.RecoTrackNHits[itrack]; h++) {
            ...
            // std::cout<<"itrack: "<<itrack<<", h: "<<h<<", out of n hits:
            // "<<truth.RecoTrackNHits[itrack]<<", xyz:
            // "<<x<<","<<y<<","<<z<<std::endl;
          }
          for (int h = 0; h < truth.RecoTrackNHits[itrack]; h++) {
...
int main(int argc, char *argv[]) {
  ...
  std::string directoryPath = "/exp/dune/data/users/" +
                              ...
                              "/dune-tms/Validation/Plot_True_Track_Length/";
