...
def in_between(x_lar_start, x_lar_end, y_lar_start, y_lar_end, z_lar_start, z_lar_end):
    if (LAR_START[0] < x_lar_start and LAR_START[1] < y_lar_start and LAR_START[2] < z_lar_start and LAR_END[
        0] > x_lar_end
            and LAR_END[1] > y_lar_end and LAR_END[2] > z_lar_end):
        return True
    else:
        return False
...
def run(truth, f, outfilename, nmax=-1):
    ...
    for i in range(truth.GetEntries()):
        ...
        for index, particle in enumerate(truth.PDG):
            ...
            if truth.PDG[index] == 13:
                ...
                KE_muon = truth.Muon_TrueKE  # TODO: we also want KE of muon entering TMS!
    ...
    return muon_signed_distances, amuon_signed_distances, [correct_muon_ke, total_muon_ke], [correct_amuon_ke,
                                                                                             total_amuon_ke]
...
def validate_then_run(args):
    ...
    if outdir == "":
        ...
        if not any(location in outdir for location in good_locations):
            print(
                f"Warning: outdir is not in list of good locations. Don't want to write root files to app area. {outdir}")
    ...
    if os.path.exists(outfilename):
        if args.allow_overwrite:
            print(
                f"Warning: A file already exists at {outfilename}, but you specified allow_overwrite. It will be overwritten")
        else:
            raise ValueError(
                f"The output file already exists at {outfilename}. Please specify another location, allow_overwrite or delete the file yourself.")
...
if __name__ == "__main__":
    ...
    group.add_argument('--filename', '-f', type=list_of_strings, help="The input file, if you have a single file",
                       default="")
    parser.add_argument('--nevents', '-n', type=int, help="The maximum number of events to loop over", default=-1)
    parser.add_argument('--allow_overwrite', help="Allow the output file to overwrite",
                        action=argparse.BooleanOptionalAction)
    parser.add_argument('--preview', help="Save preview images of the histograms",
                        action=argparse.BooleanOptionalAction)
