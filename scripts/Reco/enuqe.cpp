void enuqe(std::string filename) {
  ...
  for (int i = 0; i < nentries; ++i, ++true_entry, ++reco_entry) {
    ...
    if (AllDet) {
      if (FirstHoughHit[longtrack][0] < 11134+55*2) continue;
    } else {
      if (FirstHoughHit[longtrack][0] < 11134+55*2 || FirstHoughHit[longtrack][0] > 14435) continue;  // Changed to TMS_Thick_Start for second number
    }
